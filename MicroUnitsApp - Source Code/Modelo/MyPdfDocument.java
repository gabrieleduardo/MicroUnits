...
public class MyPdfDocument {
    ...
    /*
     * Tipos de pdf a serem gerados
     */
    private static final int basico = 1;
    ...
    private static final Font verde = new Font(getUnicodeFont(), 14,
            Font.NORMAL, new BaseColor(8, 138, 75));
    ...
    /**
     * Cria um arquivo PDF da analise de pausas o fixações
     *
     * @param path - Diretório dos Arquivos XML
     * @param arquivo - Nome do Arquivo PDf a ser gerado
     * @param pausas - Tempo de Pausa
     * @param tipo - Tipo do Documento Gerado - 1 Básico - 2 Fix
     * @throws java.lang.Exception
     */
    public static void create(String path, String arquivo, Integer pausas, Integer tipo) throws Exception {
        try {
            ...
            if (tipo == basico) {
                createA(nomeArquivos, path, arquivo, pausas);
            }

            if (tipo == fix) {
                createB(nomeArquivos, path, arquivo, pausas);
            }
        } catch (Exception ex) {
            throw new Exception("Nenhum arquivo XML no Diretório");
        }
    }

    /**
     * Cria um arquivo PDF básico, contendo apenas o campo Key
     */
    private static void createA(String[] nomeArquivos, String path, String arquivo, Integer pausas) throws Exception {
        ...
        for (String st : nomeArquivos) {
            if (st.endsWith(".xml")) {
                ...
                stList.add(myDoc.analisarPausas(pausas));
            }
        }
    ...
    /**
     * Criar arquivos pdf com análise de fixações
     */
    private static void createB(String[] nomeArquivos, String path, String arquivo, Integer pausas) throws Exception {
        ArrayList<ArrayList<String>> stList = new ArrayList<>();
        String docName;
        String[] listPDF;
        String[] listXML;
        ...
        for (String st : nomeArquivos) {
            if (st.endsWith(".xml")) {
                ...
                MyDoc myDoc = AnotherParse.parseDocument(path + getSlash() + st);
                stList.clear();
                stList.add(myDoc.analisarPausasFix(pausas));
                listPDF = arquivo.split(".pdf");
                listXML = st.split(".xml");
                docName = listPDF[0] + listXML[0].toUpperCase() + ".pdf";
                System.out.println("Write(" + docName + ")");
    ...
    private static void write(ArrayList<ArrayList<String>> stList, String filename) throws DocumentException, FileNotFoundException {
        ...
        try {
            ...
            throw new DocumentException("Houve uma falha na leitura do Documento");
        } catch (FileNotFoundException ex) {
            throw new FileNotFoundException("Arquivo não " + filename + " encontrado");
        } finally {
            if (document != null) {
                document.close();
            }
     ...
     * Fonte: http://www.vogella.com/tutorials/JavaPDF/article.html
     */
    ...
    private static BaseFont getUnicodeFont() {
        try {
            ...
            Logger.getLogger(MyPdfDocument.class
